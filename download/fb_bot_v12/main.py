#!/usr/bin/env python3
"""
FB Auto-Comment Bot v12 - Live View + Cookie Auth Edition
- Cookie-based authentication (export FB='cookie')
- Live View: MJPEG stream browser screen (lightweight, JPEG, ~1fps)
...
import json, time, random, os, re, sys, datetime, threading, io, traceback, queue as _queue
...
# Live View config - optimized for 2GB RAM
LIVE_FPS = 0.8
LIVE_QUALITY = 25
...
# ===========================================================
#  SHARED STATE (minimal - no base64 screenshots stored)
# ===========================================================
S = {
...
def capture_frame(page):
    try:
        ss_bytes = page.screenshot(type="jpeg", quality=LIVE_QUALITY)
        sset("live_frame", ss_bytes)
    except:
        pass
        ...
        try:
            ...
            while True:
                ...
                if action == "load_cookie":
                    ...
                    sset("live_frame", None)
                    slog("Semua data berhasil direset", "INFO")

            # Live capture loop
            now = time.time()
            if page and sget("live_clients") > 0 and (now - _last_capture) >= (1.0 / LIVE_FPS):
                _last_capture = now
                capture_frame(page)
...
<title>FB Bot v12 - Live View</title>
<style>
...
<div class="wrap">
<h1>FB Auto-Comment Bot v12</h1>
<p class="sub">Live View + Cookie Auth - Stream Mode</p>

<div class="card">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
    <div>
      <p style="font-weight:600" id="pT">Initializing...</p>
      <p id="mT" style="color:#8b949e;font-size:.78rem">Loading...</p>
    </div>
...
@app.route("/api/live/on", methods=["POST"])
def api_live_on():
    with S_lock:
        S["live_clients"] += 1
    return jsonify(get_sd())

@app.route("/api/live/off", methods=["POST"])
def api_live_off():
    with S_lock:
...
# ===========================================================
def main():
    pr(f"\n{C.CY}{C.B}  FB AUTO-COMMENT BOT v12 - LIVE VIEW{C.R}")
    pr(f"  {C.GR}Live View | Cookie Auth | Stream Mode | No Limits{C.R}\n")
    ...
    pr(f"  Live FPS: {C.D}{LIVE_FPS} (quality {LIVE_QUALITY}%){C.R}")
    pr(f"  Viewport: {C.D}{LIVE_VIEWPORT['width']}x{LIVE_VIEWPORT['height']}{C.R}")
    ...
    pr(f"  {C.G}{C.B}DASHBOARD:{C.R}")
    pr(f"  {C.CY}http://localhost:{WEB_PORT}{C.R}")
    pr(f"  {C.CY}http://localhost:{WEB_PORT}/live{C.R} (stream langsung){C.R}\n")
